...
| `@cq/ledger-mcp` | Standalone MCP server exposing the 58-tool ledger surface over **stdio** or **Streamable HTTP**. |
...
`list_milestone_items`, `archive_milestone`, `archive_terminal_items`, `snapshot`, `workset`, `reopen_item`,
...
The 58-tool ledger surface uses a single breaking wire-response contract:
