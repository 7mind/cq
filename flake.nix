      ...
      let
        ...
        sourceRevision =
          if
            ...
            && builtins.match "^[0-9a-f]{40}$" self.rev != null
            && !(self ? dirtyRev)
          then
            self.rev
          else
            throw "cq packages require a clean full self.rev for immutable build provenance";
        ...
        embedServerClosure = ''
          ...
          rm -f "$WORKSPACE/packages/ledger-mcp/src/buildProvenance.gen.ts"
          cat > "$WORKSPACE/packages/ledger-mcp/src/buildProvenance.gen.ts" <<'EOF'
/** Generated by flake.nix; the Nix store makes this module read-only. */
export const PACKAGED_BUILD_COMMIT = "${sourceRevision}" as const;
EOF
          chmod 0444 "$WORKSPACE/packages/ledger-mcp/src/buildProvenance.gen.ts"
          rm -rf \
          ...
          for dep in zod yaml unified remark-frontmatter remark-parse remark-stringify minisearch typescript bun-types; do
        ...
        checks =
          {
            ...
            codex-cq-skills =
              ...
              pkgs.runCommand "codex-cq-skills" { } ''
                ...
                test "$(find ${codexPromptRoot}/roles -type f -name '*.md' | wc -l)" -eq 26
            ...
            claude-prompt-root = pkgs.runCommand "claude-prompt-root-check" { } ''
              test "$(find ${claudePromptRoot}/roles -type f -name '*.md' | wc -l)" -eq 26
            ...
            pi-prompt-root = pkgs.runCommand "pi-prompt-root-check" { } ''
              ...
              test "$(find ${piPromptRoot}/roles -type f -name '*.md' | wc -l)" -eq 26
