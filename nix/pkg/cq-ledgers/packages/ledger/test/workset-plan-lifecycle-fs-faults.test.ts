...
async function open(
  ...
  const store = createWorksetGuardedPlanLifecycleStore({
    ...
    runPlanLifecycleTransaction: (mutate) => rawStore.runAtomicWorksetPlanLifecycleMutation(mutate),
  });
  await store.init();
  stores.push(store);
  return store;
...
describe("workset plan lifecycle filesystem faults [T1968]", () => {
  for (const fault of [
    {
      name: "write",
      error: () => new Error("injected guarded plan write failure"),
    },
    {
      name: "rename",
      error: () =>
        Object.assign(new Error("injected guarded plan rename failure"), {
          code: "EXDEV",
        }),
    },
  ]) {
    it(`${fault.name} failure restarts at the prestate and retries as new`, async () => {
      ...
      await expect(store.publishPlanDraft(publishInput)).rejects.toThrow(
        `injected guarded plan ${fault.name} failure`,
      );
      armed = false;
