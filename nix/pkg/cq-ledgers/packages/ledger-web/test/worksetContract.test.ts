...
async function buildProductionClient(): Promise<WorksetClientContractFixture> {
  ...
  const server = createManagementLedgerMcpServer({
    store,
    displayName: "ledger-web-workset-contract",
  });
  const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  await server.connect(serverTransport);
  const rawClient = new Client(
    { name: "ledger-web-workset-contract", version: "0.0.1" },
    { capabilities: {} },
  );
...
runWorksetClientContract({
  name: "ledger-web McpLedgerClient + production ledger-mcp",
