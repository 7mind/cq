  ...
  "fail may carry verified or unresolvable baseVerification with a closed reason and null SHAs where unobserved"
    ...
    inputSchema: {
      ...
      "properties": {
        ...
        "inheritedGitReceipts": {
          ...
          "description": "Server-bound immutable receipt prefix from terminal prior generations. Callers must omit it; a brokered retry prepends it unchanged to the final receipt chain without replaying Git effects."
        },
    ...
    outputSchema: {
      ...
      "properties": {
        ...
        "gitReceipts": {
          "type": "array",
          "description": "Durable dispatch-bound broker receipts retained when the worker performed brokered commits. The ordinary arm (no gitLineage) keeps a non-empty complete chain; the guarded-rebase arm reports only the fresh post-rebase suffix, which may be empty only in the server-resolved exact-tip mode.",
          "items": {
            "type": "object",
            "properties": {
              "kind": {
