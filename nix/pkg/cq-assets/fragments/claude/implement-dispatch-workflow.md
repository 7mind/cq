...
> `CQ_SUBAGENT` with the worker role, the §K4
...
> Apply the blocking consumed-only rule in §2 before interpreting the
...
> fetch failure enters the §5 bailout. Only the fetched consumed body is a
