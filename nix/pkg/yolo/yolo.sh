...
# while rejecting every other verb.
TMUX_BIND_ARGS=()
CLIP_PROXY_DIR=""
CLIP_BROKER_PID=""
CLIP_SHIM_DIR=""
if [[ -n "${TMUX:-}" && -n "${YOLO_CLIPBOARD_PROXY:-}" ]]; then
  _tmux_sock="${TMUX%%,*}"
  if [[ -S "$_tmux_sock" ]]; then
    ...
    if [[ -n "$CLIP_BROKER_PID" && -S "$_clip_sock" ]]; then
      ...
      # Keep $TMUX so agents still choose the tmux clipboard path; the host
      # socket path it names is intentionally NOT bound, so a non-shim tmux
      # binary fails closed rather than reaching the host server.
    else
